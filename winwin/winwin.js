...
(function($, window, document, undefined) {
  ...
  var WinWin = {
    ...
    '_registerNewEvent': function(eventName) {
      ...
      // set the dirty flag to true so it fires on the first go
      winwin.flaggedEvents[eventName] = true;
      ...
      for (i = dirtyEvents.length - 1; i >= 0; i--) {
        ...
        if (handlerQueue && handlerQueue.length) {
          ...
          for (j = handlerQueue.length - 1; j >= 0; j--) {
            ...
            handlerFunc && typeof handlerFunc === 'function' && handlerFunc();
          }
        }
      }
